/**
 * PriceTrajectoryChart.jsx — Standalone component for Price Trajectory Chart
 * 
 * Renders the Price Trajectory Chart — a new visual component.
 * This is NOT a redesign of any existing chart in the project.
 * This is NOT connected to any existing chart component or charting library
...
/**
 * Custom tooltip for the Price Trajectory Chart
 */
    ...
    return (
      <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          {isPredicted ? 'AI Predicted' : 'Historical'}: {currency === 'inr' ? '₹' : currency === 'eur' ? '€' : '$'}{data.value?.toLocaleString()}
        </p>
...
export default function PriceTrajectoryChart({ 
  historicalData = [], 
  predictedData = [], 
  productName = '',    // was coinName — renamed for DeltaDrop (Indian products)
  coinName = '',       // kept for backward compatibility
  currency = 'inr',   // default INR for Indian e-commerce
  className = ''
}) {
  const displayName = productName || coinName
    ...
    <div className={`w-full ${className}`}>
      ...
      <div className="mt-4 text-center">
        <p className="text-sm text-gray-500 dark:text-gray-400">
          ...
          {historicalData.length} historical points • {predictedData.length} AI predictions
        </p>
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
          Historical data: Solid blue area | AI Predictions: Dashed orange line
        </p>
      </div>
    </div>
  )
